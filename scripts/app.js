const interviewSections = [
  {
    ...
    questions: [
      "Qual é a principal mensagem que você gostaria que o mercado associasse à Balmoral quando falamos de ESG?",
  ...
  },
  {
    ...
    questions: [
      "Como a cultura da Balmoral incentiva práticas responsáveis no dia a dia dos times?",
  ...
  },
  {
    ...
    questions: [
      ...
      "O que torna o case da Balmoral replicável, escalável ou inspirador para outras empresas?",
  ...
  },
  {
    ...
    questions: [
      "Quais são as próximas prioridades ESG da Balmoral para os próximos 12 a 24 meses?",
...
function renderInterviewSections() {
  interviewSections.forEach((section, sectionIndex) => {
    ...
    section.questions.forEach((question) => {
      list.append(createElement("li", null, question));
    });
...
function buildInterviewText(sections = interviewSections) {
  const header = "Roteiro de Entrevista C-Level | Troféu ESG Balmoral";
...
function handleAgendaSubmit(event) {
  ...
  const executive = formData.get("executive") || "C-level";
  const company = formData.get("company") || "Balmoral";
  ...
  generatedAgenda.textContent = [
    `Pauta sugerida para ${executive} | ${company}`,
    "Objetivo: coletar visão executiva, evidências e compromissos ESG para o Troféu ESG.",
    "Perguntas prioritárias:",
    ...agendaQuestions.map((question, index) => `${index + 1}. ${question}`),
